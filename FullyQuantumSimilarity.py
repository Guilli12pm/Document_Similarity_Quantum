...
def evaluate(model, classifier, inputs):
    hidden1 = tq.QuantumDevice(num_qubits, bsz=batch_size, device=device)
    hidden2 = tq.QuantumDevice(num_qubits, bsz=batch_size, device=device)
...
for epoch in range(EPOCH):
    ...
    print(f'Epoch: {epoch}, Loss:{loss}, Train Accuracy: {train_accuracy * 100:.2f}% - time start: {timeSince(start)}')

    # Calculate accuracy on test dataset
    test_accuracy = calculate_accuracy(model, classifier, test_dataset)
    print(f'Epoch: {epoch}, Loss:{loss}, Test Accuracy: {test_accuracy * 100:.2f}% - time start: {timeSince(start)}')